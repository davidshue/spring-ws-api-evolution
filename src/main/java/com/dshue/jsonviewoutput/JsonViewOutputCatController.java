...
class JsonViewOutputCatController {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/{catId}", produces = "application/vnd.dshue.cat.v1+json")
    @ResponseBody
    Cat readV1(@PathVariable Integer catId) {
        return new Cat();
    }

    @RequestMapping(method = RequestMethod.GET, value = "/{catId}", produces = "application/vnd.dshue.cat.v2+json")
    @ResponseBody
